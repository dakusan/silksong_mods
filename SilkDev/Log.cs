...
public static class Log
{
	...
	private static void ILog(object[] Objs, bool TimeStamp, LogLevel LLevel)
	{
		...
		foreach(ILoggerLog L in AdditionalLoggers.Values)
			L.Log($"[{LLevel}] {OutStr}");
	}

	//Extra loggers
	private static readonly System.Collections.Generic.Dictionary<string, ILoggerLog> AdditionalLoggers=[];
	public static void AddLogger(string Name, ILoggerLog L)
	{
		if(AdditionalLoggers.TryGetValue(Name, out ILoggerLog Log))
			Log.Dispose();
		AdditionalLoggers[Name]=L;
	}
	public static bool RemoveLogger(string Name)
	{
		bool HasKey=AdditionalLoggers.Remove(Name, out ILoggerLog Log);
		if(HasKey)
			Log.Dispose();
		return HasKey;
	}
