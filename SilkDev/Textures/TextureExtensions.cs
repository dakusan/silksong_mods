...
public static class TextureExtensions
{
	...
	public static Rect ConvertTexCoords(this Rect R, Texture Tex) => new(R.position/Tex.Size, R.size/Tex.Size);
	...
	public static Texture2D ToReadable(this Texture Tex, Rect? TexCoords=null, Vector2? ResizeDimensions=null)
	{
		...
		if(RT2!=null) {
			using TypedDisposer<Texture2D>? TextToCopyFrom=
				  (Tex is not Texture2D Tex2D) || !UnityEngine.Experimental.Rendering.GraphicsFormatUtility.IsCompressedFormat(Tex2D.format) ? null
				: Tex2D.ToReadable().Disposable; //If compressed then we are forced to create a copy of the whole texture first before extracting what we need //TODO: We could theoretically do compressed textures via multiples of 4 receivers for copies
