...
public class Config
{
	...
	public readonly ConfigEntryT<bool> BlockGameInput, ForceShowMouse, ShowMessageWhenGameInputBlocked, SkipIntro, ForceStackTrace, BlockMouse_UnityExplorer, BlockMouse_BepInExConfig, RunExtractAllTextures;
	...
	internal Config(ConfigFile PConfig)
	{
		...
		Key_BlockInput=Con.Bind(Title, "Block game input shortcut key", new KeyboardShortcut(KeyCode.F4));
		ShowMessageWhenGameInputBlocked=Con.Bind(Title, "Show message when input blocked", true);
		...
		Key_GameObjectSprites=Con.Bind(Title, "Get textures from sprites under your cursor", new KeyboardShortcut(KeyCode.None), "Opens the Game Object Sprites window");
