...
public class InputRepeatDelay<EmbeddedType>(float RepeatDelay=0.5f)
{
	...
	public abstract class InputType(EmbeddedType? ReturnValue=default)
	{
		...
		public static implicit operator InputType((InputDevice ID, IInputControl  IC							) v ) => new IRDInputControl		(v.ID, v.IC								);
		public static implicit operator InputType((InputDevice ID, IInputControl  IC, EmbeddedType? ReturnValue	) v ) => new IRDInputControl		(v.ID, v.IC,			v.ReturnValue	);
	...
	public bool RemoveItem(InputControl IC						) => IRDInputControl		.Remove(this, IC					);
	...
	//Monitor the input from the current active device
	public class IRDInputControl : InputType
	{
		//Keep updated the delegate to get the current InputControl
		private readonly MethodInfo? InputGetter;
		private InputDevice CurrentDevice;
		private Func<IInputControl>? GetInput;
		public IRDInputControl(InputDevice ID, IInputControl IC, EmbeddedType? Value = default) : base(Value)
		{
			//Get the Getter() MethodInfo
			InputControl=IC;
			InputGetter=(CurrentDevice=ID).GetType()
				.GetProperties(BindingFlags.Instance | BindingFlags.Public)
				.FirstOrDefault(PI => {
					try { return ReferenceEquals(PI.GetValue(CurrentDevice), IC); }
					catch { return false; }
				})?.GetGetMethod();

			//Update the getter delegate
			if(InputGetter==null)
				Log.Error("Could not bind input control property - device change will not update it.");
			else
				UpdateControl();
		}

		//Update the getter delegate after a ActiveDevice change
		private void UpdateControl() =>
			GetInput=(Func<IInputControl>)Delegate.CreateDelegate(
				typeof(Func<IInputControl>),
				CurrentDevice=InputManager.ActiveDevice,
				InputGetter!
			);

		//Get the input control, accounting for failed reflection lookup and changed ActiveDevice
		public IInputControl InputControl { get
		{
			if(GetInput==null)
				return field;

			if(CurrentDevice!=InputManager.ActiveDevice) //The InputManager.OnActiveDeviceChanged wasn’t working, so for now I’m actively polling the current device
				UpdateControl();
			return GetInput.Invoke();
		} }

		//Override/default functions
		public override bool IsPressed => InputControl.IsPressed;
		internal static bool Remove(InputRepeatDelay<EmbeddedType> Parent, InputControl IC) =>
			Parent.RemoveItem(I => I is IRDInputControl Input && Input.InputControl==IC);
	}
